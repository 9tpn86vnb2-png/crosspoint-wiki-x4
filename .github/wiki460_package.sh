...
for old in [
    ...
    s = s.replace(old, 'crosspoint-1.6.0-wiki-4.6.0-x4-UNTESTED.bin')
for old in [
    ...
    s = s.replace(old, "UI_VERSION = '1.6.0-wiki-4.6.0'")
...
if needle not in s:
    raise SystemExit('4.6.0 package: source manifest anchor not found')
s = s.replace(needle, replacement, 1)
if "NAME = 'crosspoint-1.6.0-wiki-4.6.0-x4-UNTESTED.bin'" not in s:
    raise SystemExit('4.6.0 package: artifact name replacement failed')
if "UI_VERSION = '1.6.0-wiki-4.6.0'" not in s:
    raise SystemExit('4.6.0 package: UI version replacement failed')
p.write_text(s)
PY
