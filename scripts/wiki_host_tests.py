#!/usr/bin/env python3
"""Test real WikiArchive.cpp with host storage, Arduino and zlib adapters.
Not display/device tests. --skip-real omits the published-pack download.
"""
...
class HalFile {
 ...
 explicit HalFile(const char* path) {
  auto s=std::make_shared<std::ifstream>(path,std::ios::binary);
  if(s->is_open()) { s->seekg(0,std::ios::end); bytes=uint64_t(s->tellg()); s->seekg(0); file=s; }
 }
 explicit operator bool() const { return bool(file)&&file->is_open(); }
 uint64_t size() const { return bytes; }
 bool seek(uint64_t p) { if(!file||p>bytes) return false; file->clear(); file->seekg(p); return bool(*file); }
 int read(void* p,size_t n) { if(!file) return -1; file->read(static_cast<char*>(p),n); return int(file->gcount()); }
...
class InflateReader {
 ...
 void setSource(const uint8_t* p,size_t n) { stream.next_in=const_cast<Bytef*>(p); stream.avail_in=n; }
 InflateStatus readAtMost(uint8_t* p,size_t n,size_t* produced) {
  stream.next_out=p; stream.avail_out=n;
  int result=inflate(&stream,Z_FINISH); *produced=n-stream.avail_out;
...
''')
    ...
    second=struct.unpack_from('<I',damaged,12+44+32)[0]
    struct.pack_into('<I',damaged,12+44+36,1); damaged[second]=6
...
int main(int argc,char** argv) {
 try {
  ...
  check(archive.search("Moon").text.size()==std::string("Long fixture body. ").size()*1500,"large bounded article");
...
'''.replace('INVALID_NAMES',invalid_names)
    ...
    subprocess.run(['g++','-std=c++20','-O1','-g','-Wall','-Wextra','-Werror','-fsanitize=address,undefined','-fno-omit-frame-pointer',
        '-I'+str(STUBS),'-I'+str(ROOT/'src/activities/wiki'),str(ROOT/'src/activities/wiki/WikiArchive.cpp'),
        str(WORK/'tests.cpp'),'-lz','-o',str(WORK/'tests')],check=True)
